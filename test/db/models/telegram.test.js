...
describe('test ../db/models/telegram.js', function () {
    before('create test user', async function () {
        ...
        testTgChat = await userFabric.createTelegramChatForUser(testUser.user_id, chatValue);
    });

    describe('test function getChatBy()', function () {
        it('should be found by value', function () {
            const dbTgUser = telegramModel.getChatBy(chatValue).then((tgUser) => { return JSON.stringify(tgUser) });
    ...
    describe('test function getChatByLocalId()', function () {
        it('should be created', function () {
            ...
            const dbTgUser = telegramModel.getChatByLocalId(id).then((tgUser) => { return tgUser._id });
    ...
    describe('test function getChatsList()', function () {
        it('should be 1 or more', function () {
            const tgUsersList = telegramModel.getChatsList().then((list) => { return list.length });
    ...
    describe('test function getChatsListByUser()', function () {
        it('should be 1 or more', function () {
            const tgUsersList = telegramModel.getChatsListByUser(1).then((list) => { return list.length });
            return assert.eventually.isAtLeast(tgUsersList, 1);
        });
        it('should be 0', function () {
            const tgUsersList = telegramModel.getChatsListByUser(123465789).then((list) => { return list.length });
